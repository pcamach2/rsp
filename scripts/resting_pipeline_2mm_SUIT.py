...
parser.add_option("-f", "--func",  action="store", type="string", dest="funcfile",
                  help="bxh ( or nifti ) file for functional run", metavar="/path/to/BXH")
parser.add_option("--throwaway",  action="store", type="int", dest="throwaway",
                  help="number of timepoints to dis-regard from beginning of run", metavar="4")
parser.add_option("--t1",  action="store", type="string", dest="anatfile",
                  help="bxh ( or nifti ) file for the anatomical T1", metavar="/path/to/BXH")
parser.add_option("-p", "--prefix",  action="store", type="string", dest="prefix",
                  help="prefix for all resulting images, defaults to name of input", metavar="func")
parser.add_option("-s", "--steps",  action="store", type="string", dest="steps",
                  help="comma seperated string of steps. 'all' will run everything, default is all", metavar="0,1,2,3", default='all')
parser.add_option("-o", "--outpath",  action="store", type="string", dest="outpath",
                  help="location to store output files", metavar="PATH", default='PWD')
parser.add_option("--sliceorder",  action="store", type="string", dest="sliceorder",
                  help="sliceorder if slicetime correction ( odd=interleaved (1,3,5,2,4,6), up=ascending, down=descending, even=interleaved (2,4,6,1,3,5) ).  Default is to read this from input image, if available.", metavar="string")
parser.add_option("--tr",  action="store", type="float", dest="tr_ms",
                  help="TR of functional data in MSEC", metavar="MSEC")
parser.add_option("--ref",  action="store", type="string", dest="flirtref",
                  help="pointer to FLIRT reference image if not using standard brain", metavar="FILE")
parser.add_option("--flirtmat",  action="store", type="string", dest="flirtmat",
                  help="a pre-defined flirt matrix to apply to your functional data. (ie: func2standard.mat)", metavar="FILE")
parser.add_option("--refwm",  action="store", type="string", dest="refwm",
                  help="pointer to WM mask of reference image if not using standard brain", metavar="FILE")
parser.add_option("--refcsf",  action="store", type="string", dest="refcsf",
                  help="pointer to CSF mask of reference image if not using standard brain", metavar="FILE")
parser.add_option("--refgm",  action="store", type="string", dest="refgm",
                  help="pointer to GM mask of reference image if not using standard brain", metavar="FILE")
parser.add_option("--refbrainmask",  action="store", type="string", dest="refbrainmask",
                  help="pointer to brain mask of reference image if not using standard brain", metavar="FILE")
parser.add_option("--refacpoint",  action="store", type="string", dest="refac",
                  help="AC point of reference image if not using standard MNI brain", metavar="45,63,36", default="45,63,36")
parser.add_option("--betfval",  action="store", type="float", dest="betfval",
                  help="f value to use while skull stripping. default is 0.4", metavar="0.4", default='0.4')
parser.add_option("--anatbetfval",  action="store", type="float", dest="anatbetfval",
                  help="f value to use while skull stripping ANAT. default is 0.5", metavar="0.5", default='0.5')
parser.add_option("--lpfreq",  action="store", type="float", dest="lpfreq",
                  help="frequency cutoff for lowpass filtering in HZ.  default is .08hz", metavar="0.08", default='0.08')
parser.add_option("--corrlabel",  action="store", type="string", dest="corrlabel",
                  help="pointer to 3D label containing ROIs for the correlation search. default is the 116 region AAL label file", metavar="FILE")
parser.add_option("--corrtext",  action="store", type="string", dest="corrtext",
                  help="pointer to text file containing names/indices for ROIs for the correlation search. default is the 116 region AAL label txt file", metavar="FILE")
parser.add_option("--corrts",  action="store", type="string", dest="corrts",
                  help="If using step 7b by itself, this is the path to parcellation output (default is to use OUTPATH/corrlabel_ts.txt), which will be used as input to the correlation.", metavar="FILE")
...
parser.add_option("--dvarsnumneighbors",  action="store", type="int", dest="dvarsnumneighbors",
                  help="If --dvarsthreshold is specified, then --dvarsnumnumneighbors specifies how many neighboring volumes, before and after the initially excluded volumes, should also be excluded.  Default is 0.", metavar="NUMNEIGHBORS")
...
parser.add_option("--fdnumneighbors",  action="store", type="int", dest="fdnumneighbors",
                  help="If --fdthreshold is specified, then --fdnumnumneighbors specifies how many neighboring volumes, before and after the initially excluded volumes, should also be excluded.  Default is 0.", metavar="NUMNEIGHBORS")
...
parser.add_option("--motionnumneighbors",  action="store", type="int", dest="motionnumneighbors",
                  help="If --motionthreshold is specified, then --motionnumnumneighbors specifies how many neighboring volumes, before and after the initially excluded volumes, should also be excluded.  Default is 1.", metavar="NUMNEIGHBORS")
parser.add_option("--motionpar",  action="store", type="string", dest="motionpar",
                  help="If --motionthreshold is specified, then --motionpar specifies the .par file from which the motion parameters are extracted.  If you allow this script to perform motion correction, then this option is ignored.", metavar="FILE.par")
parser.add_option("--scrubop",  action="store", choices=('and', 'or'), dest="scrubop",
                  help="If --motionthreshold, --dvarsthreshold, or --fdthreshold are specified, then --scrubop specifies the aggregation operator used to determine the final list of excluded volumes.  Default is 'or', which means a volume will be excluded if *any* of its thresholds are exceeded, whereas 'and' means all the thresholds must be exceeded to be excluded.")
parser.add_option("--powerscrub", action="store_true", dest="powerscrub",
                  help="Equivalent to specifying --fdthreshold=0.5 --fdnumneighbors=0 --dvarsthreshold=0.5% --dvarsnumneigbhors=0 --scrubop='and', to mimic the method used in the Power et al. article.  Any conflicting options specified before or after this will override these.", default=False)
parser.add_option("--scrubkeepminvols",  action="store", type="int", dest="scrubkeepminvols",
                  help="If --motionthreshold, --dvarsthreshold, or --fdthreshold are specified, then --scrubminvols specifies the minimum number of volumes that should pass the threshold before doing any correlation.  If the minimum is not met, then the script exits with an error.  Default is to have no minimum.", metavar="NUMVOLS")
parser.add_option("--fcdmthresh",  action="store", type="float", dest="fcdmthresh",
                  help="R-value threshold to be used in functional connectivity density mapping ( step8 ). Default is set to 0.6. Algorithm from Tomasi et al, PNAS(2010), vol. 107, no. 21. Calculates the fcdm of functional data from last completed step, inside a dilated gray matter mask", metavar="THRESH", default=0.6)
parser.add_option("--cleanup",  action="store_true",
                  dest="cleanup", help="delete files from intermediate steps?")
...
class RestPipe:
    ...
    def initialize(self):
        ...
        if options.anatfile is not None:
            if not (os.path.isfile(options.anatfile)):
                ...
                if fileExt == '.bxh':
                    ...
                    thisproc = subprocess.Popen(
                        ["/bxh/bin/fslwrapbxh " + self.t1nii], shell=True).wait()
        ...
        if options.flirtref is not None:
            ...
            logging.info('Using ' + options.refac +
                         ' for AC point/centroid calculation')
            ...
            self.refwm = os.path.join(
                self.basedir, 'data', 'MNI152_T1_2mm_brain_pve_2.nii.gz')
            self.refcsf = os.path.join(
                self.basedir, 'data', 'MNI152_T1_2mm_brain_pve_0.nii.gz')
            self.refgm = os.path.join(
                self.basedir, 'data', 'MNI152_T1_2mm_brain_pve_1.nii.gz')
            self.refac = str(options.refac)
            self.refbrainmask = os.path.join(
        ...
        if ('0' in self.steps) and (self.origbxh is None) and (self.thisnii is not None):
            ...
            if self.tr_ms is not None:
                logging.info(
                    'requesting step0, but no bxh provided.  Creating one from ' + self.thisnii)
                thisproc = subprocess.Popen(
                    ["/bxh/bin/fslwrapbxh " + self.thisnii], shell=True).wait()

                tmpfname = re.split('(\.nii$|\.nii\.gz$)',
                                    self.thisnii)[0] + ".bxh"
                ...
                logging.info(
                    "Please provide --tr option when starting from nifti, we don't trust TR derrived from existing nifti files.")
                raise SystemExit()
        ...
        if options.corrlabel is not None:
            ...
            self.corrlabel = os.path.join(
                self.basedir, 'data', 'aal_MNI_V4.nii')
            self.corrtext = os.path.join(
                self.basedir, 'data', 'aal_MNI_V4.txt')
        ...
        if self.dvarsthreshold is not None:
            ...
            try:
                ...
                logging.error(
                    "--dvarsthreshold must be a floating-point number (optionally followed by '%')")
                raise SystemExit()
        ...
        if self.motionthreshold != None:
            if '2' not in self.steps:
                if options.motionpar == None:
                    logging.info(
                        "--motionpar option is required when using --motionthreshold if you are skipping the motion correction step (step 2).")
                    raise SystemExit()
                self.mcparams = options.motionpar
        ...
        if self.origbxh is not None:
            popenobj = subprocess.Popen(
                ['/bxh/bin/dumpheader', self.origbxh], stdout=subprocess.PIPE)
            (stdoutdata, stderrdata) = popenobj.communicate()
            lines = stdoutdata.splitlines()
            for line in lines:
                willrotate = False
                mobj = re.search(
                    "Dimension.*\((\w)\):\s*([-0-9.]+)(\S+) to ([-0-9.]+)(\S+), ([0-9]+) steps(, direction \(([-0-9.]+), ([-0-9.]+), ([-0-9.]+)\))?", line)
                if mobj:
                    (dimname, firstpos, firstunits, lastpos, lastunits,
                     numsteps, dirclause, dirR, dirA, dirS) = mobj.groups()
                    if '0' in self.steps and '1' in self.steps and (dimname == 'x' or dimname == 'y' or dimname == 'z') and dirR != None and dirA != None and dirS != None:
                        ...
                        dirS = float(dirS)
                        dirmax = dirR
                        if abs(dirA) > abs(dirmax):
                            dirmax = dirA
                        if abs(dirS) > abs(dirmax):
                            dirmax = dirS
                        if ((dimname == 'x' and dirR != dirmax) or
                            (dimname == 'y' and dirA != dirmax) or
                                (dimname == 'z' and dirS != dirmax)):
                            sys.stderr.write(
                                "ERROR: reorientation will change slice dimension and so slice timing correction will not work!\n")
                            raise SystemExit()
                    if dimname == 'x':
                        ...
                        if self.tr_ms is None:
                            self.tr_ms = (float(lastpos) -
                                          float(firstpos)) / self.tdim
                            if firstunits == 'ms':
                                pass
                            elif firstunits == 's':
                                self.tr_ms *= 1000.0
                                ...
                                sys.stderr.write(
                                    "Unexpected temporal units in image header: '%s'" % firstunits)
                                raise SystemExit()
                ...
                if mobj:
                    ...
                    testpath = os.path.join(
                        '/'.join(self.origbxh.split('/')[0:-1]), fname)
                    #testpath = fname;
            ...
            if (self.thisnii is not None) and (self.tr_ms is not None):
                ...
                if len(thisshape) == 4:
                    ...
                    logging.info(
                        "Functional data has incorrect dimensions. Expected 4D, received : " + str(len(thisshape)) + " D")
                    raise SystemExit()
        ...
        if '0' not in self.steps and self.needfunc:
            if self.thisnii is None:
                ...
                thisprocstr = str(
                    "/bxh/bin/bxh2analyze --overwrite --niigz -s " + self.origbxh + " " + newfile)
            ...
            if self.t1nii is None and self.t1bxh is not None:
                ...
                thisprocstr = str(
                    "/bxh/bin/bxh2analyze --overwrite --niigz -s " + self.t1bxh + " " + newfile)
                subprocess.Popen(thisprocstr, shell=True).wait()
                if os.path.isfile(newfile + ".nii.gz"):
            ...
            if re.search('\d+\,+', str(self.sliceorder)):
                ...
                if (re.search('(odd|even|up|down)', options.sliceorder)) and (self.slicefile is None):
                    ...
                    if self.zdim is not None:
                        slicefile = os.path.join(
                            self.outpath, 'sliceorder.txt')
                        f = open(slicefile, 'w')
                    ...
                    logging.info(
                        "sliceorder is incorrectly defined. use odd/even/up/down.")
                    raise SystemExit()
            else:
                logging.info(
                    "slice order not found. please use --sliceorder option")
                raise SystemExit()
    ...
    def step0(self):
        ...
        tempfile = os.path.join(self.tmpdir, ''.join(random.choice(
            string.ascii_uppercase + string.digits) for x in range(10)) + '.bxh')
        thisprocstr = str("/bxh/bin/bxhreorient --orientation=LAS " +
                          self.origbxh + " " + tempfile)
        logging.info('running: ' + thisprocstr)
        subprocess.Popen(thisprocstr, shell=True).wait()

        if self.throwaway is not None:
            logging.info('disregarding acquisitions')
            thisprocstr = str("/bxh/bin/bxhselect --overwrite --timeselect " +
                              str(self.throwaway) + ": " + tempfile + " " + tempfile)
            logging.info('running: ' + thisprocstr)
            subprocess.Popen(thisprocstr, shell=True).wait()
        ...
        if os.path.isfile(tempfile):
            ...
            thisprocstr = str(
                "/bxh/bin/bxh2analyze --overwrite --niigz -s " + tempfile + " " + newfile)
            logging.info('running: ' + thisprocstr)
            subprocess.Popen(thisprocstr, shell=True).wait()
            if os.path.isfile(newfile + ".nii.gz"):
                self.thisnii = newfile + ".nii.gz"
    ...
    def step1(self):
        ...
        thisprocstr = str("slicetimer -i " + self.thisnii + " -o " + newfile +
                          " -r " + str(self.tr_ms / 1000) + " --ocustom=" + self.slicefile)
        logging.info('running: ' + thisprocstr)
        subprocess.Popen(thisprocstr, shell=True).wait()
    ...
    def step2(self):
        ...
        thisprocstr = str("mcflirt -in " + self.thisnii +
                          " -o " + newfile + " -plots")
        logging.info('running: ' + thisprocstr)
        subprocess.Popen(thisprocstr, shell=True).wait()
        ...
        if os.path.isfile(newfile + ".nii.gz") and os.path.isfile(newfile + ".par"):
            ...
            thisprocstr = str("fsl_tsplot -i " + self.mcparams +
                              " -t 'MCFLIRT estimated rotations (radians)' -u 1 --start=1 --finish=3 -a x,y,z -w 640 -h 144 -o " + newfile + "_rot.png")
            logging.info('running: ' + thisprocstr)
            subprocess.Popen(thisprocstr, shell=True).wait()
            thisprocstr = str("fsl_tsplot -i " + self.mcparams +
                              " -t 'MCFLIRT estimated translations (mm)' -u 1 --start=4 --finish=6 -a x,y,z -w 640 -h 144 -o " + newfile + "_trans.png")
            logging.info('running: ' + thisprocstr)
            subprocess.Popen(thisprocstr, shell=True).wait()
            ...
            for index in range(6):
                ...
                # BPS BPS -- NEED TO ADD BACKWARD DERIVATIVE

            logging.info('starting linear regression')
            tmp_mean = np.mean(data1, axis=3)
            shape = data1.shape
            data1v = data1.reshape(
                (shape[0] * shape[1], shape[2], shape[3])).transpose((1, 2, 0))
    ...
    def step3(self):
        ...
        thisprocstr = str("fslmaths " + self.thisnii +
                          " -Tmean " + os.path.join(self.outpath, 'mean_func'))
        logging.info('running: ' + thisprocstr)
        subprocess.Popen(thisprocstr, shell=True).wait()

        # now skull strip the mean
        thisprocstr = "bet " + os.path.join(self.outpath, 'mean_func') + " " + os.path.join(
            self.outpath, 'mean_func_brain') + " -f " + str(self.betfval) + " -m"
        logging.info('running: ' + thisprocstr)
        subprocess.Popen(thisprocstr, shell=True).wait()

        # now mask full run by results
        thisprocstr = str("fslmaths " + self.thisnii + " -mas " +
                          os.path.join(self.outpath, 'mean_func_brain_mask') + " " + newfile)
        logging.info('running: ' + thisprocstr)
        subprocess.Popen(thisprocstr, shell=True).wait()
        ...
        if self.t1nii is not None:
            ...
            newfile = os.path.join(self.outpath, newprefix)
            thisprocstr = str("bet " + self.t1nii + " " +
                              newfile + " -f " + str(self.anatbetfval))
            logging.info('running: ' + thisprocstr)
            subprocess.Popen(thisprocstr, shell=True).wait()
    ...
    def step4(self):
        ...
        if self.flirtmat is not None:
            ...
            logging.info('applying transformation matrix ' +
                         self.flirtmat + ' to 4D data')
            thisprocstr = str("flirt -in " + self.thisnii + " -ref " + self.flirtref +
                              " -applyxfm -init " + self.flirtmat + " -out " + newfile)
            logging.info('running: ' + thisprocstr)
            subprocess.Popen(thisprocstr, shell=True).wait()
            ...
            thisprocstr = str("flirt -ref " + self.t1nii + " -in " + self.thisnii + " -out " + os.path.join(self.outpath, 'func2t1') + " -omat " +
                              os.path.join(self.outpath, 'func2t1.mat') + " -cost corratio -dof 6 -searchrx -90 90 -searchry -90 90 -searchrz -90 90 -interp trilinear")
            logging.info('running: ' + thisprocstr)
            subprocess.Popen(thisprocstr, shell=True).wait()
            ...
            thisprocstr = str("convert_xfm -inverse -omat " + os.path.join(
                self.outpath, 't12func.mat') + " " + os.path.join(self.outpath, 'func2t1.mat'))
            logging.info('running: ' + thisprocstr)
            subprocess.Popen(thisprocstr, shell=True).wait()

            # flirt the t1 to standard
            logging.info('flirt t1 to standard')
            thisprocstr = str("flirt -ref " + self.flirtref + " -in " + self.t1nii + " -out " + os.path.join(self.outpath, 't12standard') + " -omat " +
                              os.path.join(self.outpath, 't12standard.mat') + " -cost corratio -dof 12 -searchrx -90 90 -searchry -90 90 -searchrz -90 90 -interp trilinear")
            logging.info('running: ' + thisprocstr)
            subprocess.Popen(thisprocstr, shell=True).wait()
            if os.path.isfile(os.path.join(self.outpath, ('t12standard' + '.nii.gz'))):
                self.t1nii = os.path.join(
                    self.outpath, ('t12standard' + '.nii.gz'))
            else:
            ...
            thisprocstr = str("convert_xfm -inverse -omat " + os.path.join(self.outpath,
                                                                           'standard2t1.mat') + " " + os.path.join(self.outpath, 't12standard.mat'))
            logging.info('running: ' + thisprocstr)
            subprocess.Popen(thisprocstr, shell=True).wait()

            # compute the func2standard mat
            logging.info(
                'computing func2standard.mat from t12standard.mat func2t1.mat')
            thisprocstr = str("convert_xfm -omat " + os.path.join(self.outpath, 'func2standard.mat') + " -concat " +
                              os.path.join(self.outpath, 't12standard.mat') + " " + os.path.join(self.outpath, 'func2t1.mat'))
            logging.info('running: ' + thisprocstr)
            subprocess.Popen(thisprocstr, shell=True).wait()

            # apply the transform
            logging.info('creating normalized func %s' % (newprefix))
            thisprocstr = str("flirt -ref " + self.flirtref + " -in " + self.thisnii + " -out " + newfile +
                              " -applyxfm -init " + os.path.join(self.outpath, 'func2standard.mat') + " -interp trilinear")
            logging.info('running: ' + thisprocstr)
            subprocess.Popen(thisprocstr, shell=True).wait()

        ############################################### CHANGED ########################################################
        # This section changes things up a bit:
        # - A transformation matrix is generated from functional to freesurfer space.
        # - The transformation matrix is inverted
        # - The transformation is applied using "nearestneighbour" interpolation to the wm/csf masks, gm/seg masks for completeness,
        #  and to the parcellation mask.
        # These steps are taken in an attempt to reduce the computational cost of the pipeline, while preserving the integrity of the original functional space.
        else:


            # use the functional data to get the func2free matrix
            thisprocstr = str("flirt -in " + self.thisnii + " -ref " + self.flirtref + " -out " + os.path.join(self.outpath, 'func2free') + " -omat " + os.path.join(
                self.outpath, 'func2free.mat') + " -bins 256 -cost corratio -searchrx -90 90 -searchry -90 90 -searchrz -90 90 -dof 12 -interp trilinear")
            logging.info('running: ' + thisprocstr)
            subprocess.Popen(thisprocstr, shell=True).wait()

            # compute the func22mm mat
            logging.info(
                'computing func22mm.mat from free22mm.mat func2free.mat')
            thisprocstr = str("convert_xfm -omat " + os.path.join(self.outpath, 'func22mm.mat') + " -concat " +
                              os.path.join(os.path.dirname(os.path.realpath(__file__)),'eye.mat') + " " + os.path.join(self.outpath, 'func2free.mat'))
            logging.info('running: ' + thisprocstr)
            subprocess.Popen(thisprocstr, shell=True).wait()

            if os.path.isfile(os.path.join(self.outpath, 'func22mm.mat')):
                # apply the transform
                logging.info('creating normalized func %s' % (newprefix))
                thisprocstr = str("flirt -ref " + os.path.join(os.path.dirname(os.path.realpath(__file__)),'2mm.nii.gz') + " -in " + self.thisnii + " -out " + newfile +
                                  " -applyxfm -init " + os.path.join(self.outpath, 'func22mm.mat') + " -interp trilinear")
                logging.info('running: ' + thisprocstr)
                subprocess.Popen(thisprocstr, shell=True).wait()

                # Apply transformation to refcsf.nii.gz
                logging.info('transforming csf mask to 2mm space')
                thisprocstr = str("flirt -in " + self.refcsf + " -ref " + os.path.join(os.path.dirname(os.path.realpath(__file__)),'2mm.nii.gz') +
                                  " -applyxfm -init " + os.path.join(os.path.dirname(os.path.realpath(__file__)),'eye.mat') + " -out " + os.path.join(self.outpath, 'csf_2mm') + " -interp nearestneighbour")
                logging.info('running: ' + thisprocstr)
                subprocess.Popen(thisprocstr, shell=True).wait()

                # Apply transformation to refwm.nii.gz
                logging.info('transforming wm mask to 2mm space')
                thisprocstr = str("flirt -in " + self.refwm + " -ref " + os.path.join(os.path.dirname(os.path.realpath(__file__)),'2mm.nii.gz') +
                                  " -applyxfm -init " + os.path.join(os.path.dirname(os.path.realpath(__file__)),'eye.mat') + " -out " + os.path.join(self.outpath, 'wm_2mm') + " -interp nearestneighbour")
                logging.info('running: ' + thisprocstr)
                subprocess.Popen(thisprocstr, shell=True).wait()

                # Apply transformation to parcellation label file
                logging.info(
                    'applying transformation matrix to parcellation label file')
                thisprocstr = str("flirt -in " + self.corrlabel + " -ref " + os.path.join(os.path.dirname(os.path.realpath(__file__)),'2mm.nii.gz') +
                                  " -applyxfm -init " + os.path.join(os.path.dirname(os.path.realpath(__file__)),'eye.mat') + " -out " + os.path.join(self.outpath, 'corrlabel_2mm') + " -interp nearestneighbour")
                logging.info('running: ' + thisprocstr)
                subprocess.Popen(thisprocstr, shell=True).wait()


                # Apply transformation to fslbrain MASK
                # ADDED BRAD 
		logging.info('transforming ref brain mask to 2mm space')                
		self.refbrainhr = os.path.join(os.path.dirname(self.refcsf),'fslbrain.nii.gz')                
		thisprocstr = str("flirt -in " + self.refbrainhr + " -ref " + os.path.join(os.path.dirname(os.path.realpath(__file__)),'2mm.nii.gz') +
                                  " -applyxfm -init " + os.path.join(os.path.dirname(os.path.realpath(__file__)),'eye.mat') + " -out " + os.path.join(self.outpath, 'fslbrain_2mm.nii.gz') + " -interp trilinear")                
		logging.info('running: ' + thisprocstr)
		subprocess.Popen(thisprocstr, shell=True).wait()
            	# threshold hr brain to make mask
            	self.refbrainmaskhr = os.path.join(self.outpath, 'fslbrain_2mm.nii.gz')
            	thisprocstr = str("fslmaths " + self.refbrainmaskhr +
                          " -thr 10 " + os.path.join(self.outpath, 'fslbrain_2mm_mask.nii.gz'))
            	logging.info('running: ' + thisprocstr)
            	subprocess.Popen(thisprocstr, shell=True).wait()
                # end added BRAD
                ...
                # Reassign fields
                #self.refbrainmask = os.path.join(os.path.dirname(os.path.realpath(__file__)),'fslbrain_2mm_mask.nii.gz')
		self.refbrainmask = os.path.join(self.outpath, 'fslbrain_2mm_mask.nii.gz')
                self.refcsf = os.path.join(self.outpath, 'csf_2mm')
                self.refwm = os.path.join(self.outpath, 'wm_2mm')
                self.corrlabel = os.path.join(self.outpath, 'corrlabel_2mm')

                if os.path.isfile(os.path.join(os.path.dirname(self.refcsf), 'ref_seg.nii.gz')):
                    # Apply transformation to refw_seg.nii.gz
                    logging.info(
                        'applying transformation matrix to reference segmentation data')
                    thisprocstr = str("flirt -in " + os.path.join(os.path.dirname(self.refcsf), 'ref_seg.nii.gz') + " -ref " + os.path.join(
                        os.environ['FSLDIR'], 'data', 'standard', 'MNI152_T1_2mm_brain.nii.gz') +
                                      " -applyxfm -init " + os.path.join(os.path.dirname(os.path.realpath(__file__)),'eye.mat') + " -out " + os.path.join(self.outpath, 'ref_seg_2mm') + " -interp nearestneighbour")
                    logging.info('running: ' + thisprocstr)
                    subprocess.Popen(thisprocstr, shell=True).wait()

            else:
                logging.info('creation of flirt matrix failed.')
                raise SystemExit()
        ...
        if os.path.isfile(newfile + '.nii.gz'):
            ...
            if len(normshape) is 4:
                ...
                logging.info(
                    'normalized data has wrong shape, expecting 4D, received: ' + str(len(normshape)) + 'D')
                raise SystemExit()
        else:
    ...
    def step5(self):
        ...
        thisprocstr = str("fslmeants -i " + self.thisnii +
                          " -m " + self.refwm + " -o " + wmout)
        logging.info('running: ' + thisprocstr)
        subprocess.Popen(thisprocstr, shell=True).wait()
        ...
        for fname in [wmout, csfout]:
            if not os.path.isfile(fname):
                logging.info(
                    'could not extract timeseries, quitting: ' + fname)
                raise SystemExit()
    ...
    def step6(self):
        ...
        # build filter
        #time_all = np.linspace(0, (self.tdim * (self.tr_ms / 1000)) - .001, num=494000)
	#time_all = np.arange(0, (self.tdim * (self.tr_ms / 1000)) - .001, .001)
	#time_all = np.arange(0, (self.tdim * (self.tr_ms / 1000)) - 1, 1)
	time_all = np.arange(0, (self.tdim * (self.tr_ms)) - 1, 1, dtype=np.int32)
        logging.info(self.tdim)
	logging.info(time_all)
	logging.info(self.tr_ms)
	tr_msINT = int(self.tr_ms)
	logging.info(tr_msINT)
	#time_subTR = time_all[0:-1:self.tr_ms]
	time_subTR = time_all[0:-1:tr_msINT]
        length = len(time_subTR)
        ccc = 1.0 / (self.tr_ms / 1000) / length
        cccc = freq_cutoff / ccc
        len1 = int(round(length / 2.0 - (cccc - 2)))
        len2 = int(round(length / 2.0 + (cccc + 1)))

        tmp = np.zeros([self.tdim, 1])
        tmp[len1:len2] = 1
        tmpMA = len1 - 4
        tmpMA2 = int(round(tmpMA / 2))
        tmpAB = np.divide(np.add(1, np.cos(
            np.arange(np.pi, 2 * np.pi + ((np.pi / tmpMA) / 2), np.pi / tmpMA))), 2)
        tmpAB = tmpAB.reshape(tmpAB.shape[0], 1)
        tmpBA = np.divide(np.add(1, np.cos(
            np.arange(2 * np.pi, np.pi - ((np.pi / tmpMA) / 2), -np.pi / tmpMA))), 2)
        ...
        for cntz in range(self.zdim):
            tmp_data = data1[:, :, cntz, :]
            arr_f = np.fft.fftshift(np.fft.fft(np.fft.fftshift(
                signal.detrend(tmp_data), axes=2), axis=2), axes=2)
            arr_fc = np.multiply(arr_f, tmp.T)
            yyy00 = np.real(np.fft.fftshift(np.fft.ifft(
                np.fft.fftshift(arr_fc, axes=2), axis=2), axes=2))
            data1[:, :, cntz, :] = yyy00
        data_lowpass = data1
        del data1
        # in-place (-=, *=) operations should save memory
        ...
        if os.path.isfile(newfile):
            ...
            # not necessary, as we are not interested in functional connectivity density mapping.
    ...
    def step7a(self):
        ...
        thisprocstr = str("fslmeants -i " + self.thisnii +
                          " --label=" + self.corrlabel + " -o " + corrtxt)
        logging.info('running: ' + thisprocstr)
        subprocess.Popen(thisprocstr, shell=True).wait()
    ...
    def step7b(self):
        ...
        if os.path.isfile(corrtxt):
            ...
            labnii = nibabel.load(self.corrlabel + '.nii.gz')
            niidata = labnii.get_data()
            niihdr = labnii.get_header()
            zooms = np.array(niihdr.get_zooms())

            G = nx.Graph(atlas=str(self.corrlabel + '.nii.gz'))
            for lab in labels:
                # grab indices equal to label value
                x, y, z = (niidata == lab[0]).nonzero()
                # ascertains volume containing atlas component
                centroid = np.array(
                    [int(x.mean()), int(y.mean()), int(z.mean())])
                # finds the centroid of the component
                c_cent_str = str((centroid - aalcenter) *
                                 (zooms.astype('int')))[1:-1].strip()
                # finds the displacement vector from the anterior commissure to the centroid of the ROI
                lab.append((centroid - aalcenter) * zooms.astype('int'))
                # adds the diplacement centroid data to the label
                lab.append(0)
                G.add_node(lab[0], label=str(lab[1]),
                           centroid=c_cent_str, intensityvalue=lab[0])
                timecourse = timeseries[int(lab[0] - 1)]
                G.node[lab[0]]['timecourse'] = str(
                    timecourse.tolist()).replace(',', '').strip('\[\]')
            ...
            logging.info(
                'could not find mean timeseries matrix file "%s"', corrtxt)
            raise SystemExit()
    ...
    def step8(self):
        ...
        logging.info("running %s, masked by %s, at pearsonr value of %f" %
                     (self.thisnii, self.refgm, self.fcdmthresh))
    ...
    def scrub_motion_volumes(self, timeseries):
        motionmarkedvolstxt = os.path.join(
            self.outpath, 'motiondisp_markedvols.txt')
        ...
        if self.dvarsthreshold != None:
            ...
            maskdata = nibabel.nifti1.load(
                self.refbrainmask).get_data().astype(np.float64)
            #maskdata = nd.binary_erosion(maskdata, iterations=5)
            data = numpy.ma.array(data, mask=numpy.tile(
                (maskdata == 0)[:, :, :, np.newaxis], (1, 1, 1, data.shape[3])))
            work = data.reshape(
                [data.shape[0] * data.shape[1] * data.shape[2], data.shape[3]])
            boldmean = np.ma.mean(work)
            work = np.ma.diff(work, axis=1)
            logging.debug("work.min=%s work.max=%s work.mean=%s",
                          np.min(work), np.max(work), np.mean(work))
            #work = work / boldmean
            ...
            logging.debug("dvars.min=%s dvars.max=%s",
                          np.min(dvars), np.max(dvars))
            boldscaling = 1.0
            ...
            logging.info(' marking these volumes due to DVARS > %g: %s',
                         _dvarsthreshold * boldscaling, excludethese)
            ...
            f.write("# these are the volumes (indexed starting at 0) marked as exceeding the 'DVARS' threshold of %s\n" %
                    self.dvarsthreshold)
            np.savetxt(f, np.transpose(numpy.array(excludethese)),
                       fmt='%d', newline=' ')
            f.close()

            np.savetxt(dvarstxt, dvars, fmt='%g')
            np.savetxt(dvarspercenttxt, dvars / boldscaling, fmt='%g')
            np.savetxt(dvarsthreshtxt, [
                       _dvarsthreshold * boldscaling], fmt='%f')
        ...
        if self.fdthreshold is not None:
            ...
            logging.info(' marking these volumes due to FD > %g mm: %s',
                         self.fdthreshold, excludethese)
            for excludethis in excludethese:
                numexcls[excludethis] += 1
        ...
        if self.motionthreshold != None:
            ...
            for t in np.arange(params.shape[1] - 1):
                ...
                (qR1_s, qR1_v) = self.combine_quaternions(
                    ((qz1_s, qz1_v), (qy1_s, qy1_v), (qx1_s, qx1_v)))
                (qR2_s, qR2_v) = self.combine_quaternions(
                    ((qz2_s, qz2_v), (qy2_s, qy2_v), (qx2_s, qx2_v)))
                (qcomb_s, qcomb_v) = self.combine_quaternions(
                    ((qR2_s, -1 * qR2_v), (qR1_s, qR1_v)))
                ...
                if qcomb_s * qcomb_s != 1:
                    ...
                    # y=0]
                    rotatedpoint = np.array(
                        [50 * math.cos(angle), 50 * math.sin(angle)])
                    magrot = math.sqrt(
                        np.sum(np.power(np.subtract(np.array([50, 0]), rotatedpoint), 2)))
                ...
                maxdisplacement = math.sqrt(
                    np.inner(maxdisplacementvector, maxdisplacementvector))
            ...
            logging.info(' marking these volumes due to motion > %g mm: %s',
                         self.motionthreshold, excludethese)
            for excludethis in excludethese:
                numexcls[excludethis] += 1
            ...
            np.savetxt(f, numpy.array(excludethese)[
                       :, np.newaxis], fmt='%d', newline=' ')
            f.close()
        ...
        if len(excludedinds[0]) > 0:
            ...
            logging.info('Scrubbed the following volumes because they or their neighbors exceeded the %s threshold (first volume is 0): %s' % (
                scrubmethodstr, str(excludedinds[0])))
        ...
        if self.scrubkeepminvols != None and timeseries.shape[1] < self.scrubkeepminvols:
            logging.error('Too few volumes (%d) met the scrubbing threshold!  Exiting...' % (
                timeseries.shape[1],))
            raise SystemExit()
