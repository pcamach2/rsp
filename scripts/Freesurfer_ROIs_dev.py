...
parcellation_labels_file = aparc_labels_add.txt 
data = '/data/derivatives/restingstatepipeline'

logging.basicConfig(format='%(asctime)s %(message)s ',
                    datefmt='%m/%d/%Y %I:%M:%S %p', level=logging.INFO)

numbers = os.environ['subjects'].split()
sess = os.environ['sessions']

for subnum in numbers:
    outdir = '{}/{}/{}/freeMask'.format(data, subnum, sess)
   ...
   # create each ROI niftii file
    for index in range(parcellation_num):           # index goes 1:68
        ...
        get_ROI.inputs.in_file = '{}/{}_{}_task-rest_dir-PA_rec-uncorrected_run-1_space-MNI152NLin2009cAsym_desc-aparcaseg_dseg.nii.gz'.format(outdir,subnum,sess)
